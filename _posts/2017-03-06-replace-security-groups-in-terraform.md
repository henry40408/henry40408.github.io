...
date: 2017-03-06
slug: replace-security-groups-in-terraform
tags: terraform security-group
layout: post
draft: true
image: replace-security-group-with-terraform.png
---

![Terraform]({{ "assets/replace-security-group-with-terraform.png" | absolute_url }})
...
1.  先在 `*.tf` 檔案裡，使用 [aws_security_group](https://www.terraform.io/docs/providers/aws/r/security_group.html) 建立一個沒有 `ingress`、`egress` rule 的 dummy security group，然後新增到要抽換的 [aws_instance](https://www.terraform.io/docs/providers/aws/r/instance.html) 或 [aws_db_instance](https://www.terraform.io/docs/providers/aws/r/db_instance.html) 的 [vpc_security_group_ids](https://www.terraform.io/docs/providers/aws/r/instance.html#vpc_security_group_ids) 裡。
2.  修改 `*.tf` 檔案的 `aws_instance` 或 `aws_db_instance` block，把要抽換掉的 security group 從 `vpc_security_group_ids` 移除掉。**注意如果有公開的 port，instance 會全部斷網，如果有承諾 zero downtime 要想辦法避開**，例如先把原來 security group 的 rule 全部複製到 dummy security group，可能會需要 [aws_security_group_rule](https://www.terraform.io/docs/providers/aws/r/security_group_rule.html) 的協助。

...
![Hahow 目前的 infrastructure](1*ADmLntt1LYxyosjsXsi2vg.png)
...
![常見的藍綠部署](0*ajnoN8cfnuT3-Av0.png)
...
![Jenkins 部署並啟動包含新版 server 的 Docker container](0*3aen_zk2lO_8jE5f.png)
...
![Blue container 沒有通過健康檢查](0*j3zTqbJzLtL-z6om.png)
...
![Blue container 回到關閉的狀態，green container 繼續處理外部流量](0*XoOy-W8kSLZ9vXiU.png)
...
![Jenkins 部署修正 v2 bug 的 v3 server](0*7U0LRtmW4qoJ9Mbd.png)

一樣由 Jenkins 部署與啟動，並開始健康檢查，健康檢查的細節如前文所示，不再贅述。

#### \[健檢\] Blue container 通過健康檢查

![Blue container 通過健康檢查](0*iok9K5PSG1cSJS__.png)
...
![Jenkins 對 nginx 發出信號要求重啟](0*L3aNTbsrdiwveySq.png)
...
![開始替換 child process](0*sXDdDaAwOc26kXiy.png)

![替換掉 1 個 child process，還有 2 個](0*oz0XPyXzreZuTukj.png)

![以此類推](0*F8We6OASngg8nYGv.png)

![替換掉 2 個 child process，還有 1 個](0*90y4A89d6YOGhnFP.png)

![以此類推](0*S96ICLRDBmwkQP59.png)

![替換掉最後一個 child process](0*7k7ajwI_9LS8t-nG.png)

![完成 child process 的替換，現在外部已經無法存取舊 API server（green container）了](0*xghK08LWdA1W9HNB.png)
...
![Jenkins 要求 v1 server 在時限內關閉](0*1MYRBcuLoN_ZMxgI.png)
...
![本次藍綠部署的最終狀態](0*cyzdw815K1gGB5cQ.png)
